...
#else
  # efine PERI_BASE BCM2711_PI4_PERI_BASE
#endif
...
// The UncachedMemBlock_{alloc,free,to_physical} functions are meant to operate on these.
...
// Allocate a block of memory of the given size (which is rounded up to the next full page)
// The memory will be aligned on a page boundary and zeroed out.
...
// Direct output of data to the GPIO in a tight loop. This is the highest speed
  ...
  printf ("1) CPU: Writing to GPIO directly in tight loop Ctrl-C to exit.\n");
  for (;;) {
...
// Memory is organized as 32-Bit data to be written to the GPIO.
// We expand these using a mask to do the actual set/reset sequence.
// This is pretty compact in memory and probably the 'usual' way how you would send data to GPIO.
  ...
  printf ("2) CPU: reading word from memory, write masked to GPIO set/clr Ctrl-C to exit.\n");
...
// good prepatory step to understand the layout in the DMA case.
  ...
  // Do it. Endless loop: reading, writing.
  printf ("3) CPU: reading prepared set/clr from memory, write to GPIO Ctrl-C to exit.\n");
...
// understand (slow) DMA performance.
  ...
  struct UncachedMemBlock memblock = UncachedMemBlock_alloc (n * sizeof(*gpio_data));
  ...
  // Do it. Endless loop: reading, writing.
  printf("4) CPU: reading prepared set/clr from UNCACHED memory, write to GPIO Ctrl-C to exit.\n");
  const struct GPIOData* start = gpio_data;
  const struct GPIOData* end = start + n;
  for (;;) {
    for (const struct GPIOData* it = start; it < end; ++it) {
...
// one 32-bit set/clear operation.
  ...
  // Also, only UncachedMemBlock allows us to conveniently get the physical address.
  struct UncachedMemBlock cb_memblock = UncachedMemBlock_alloc(sizeof(struct dma_cb));
  ...
  // Setting up two transfers, each with length 4. Reading set and clr respectively.
  ...
  printf("5) DMA: Single control block per set/reset GPIO Press <RETURN> to exit (with CTRL-C DMA keeps going).");

  // 4.2.1.2
  char* dmaBase = mmap_bcm_register (DMA_BASE);
  struct dma_channel_header* channel = (struct dma_channel_header*)(dmaBase + 0x100 * DMA_CHANNEL);
...
// are using less memory than in the
  ...
  // for GPIO, each with length of the struct GPIOData, which is 16 bytes.
  ...
  printf ("6) DMA: Sending a sequence of set/clear with one DMA control block "
          "and negative destination stride Press <RETURN> to exit (with CTRL-C DMA keeps going).");
