...
//}}}
  ...
  void VsyncCallback (DISPMANX_UPDATE_HANDLE_T u, void* arg) {
    ...
    // Wake the main thread if it was sleeping to get a new frame
...
uint64_t PredictNextFrameArrivalTime() {
  ...
  // High sleep mode hacks to save battery when ~idle
  // (These could be removed with an event based VideoCore display refresh API)
  uint64_t timeNow = tick();
  ...
  // If there should have been a frame just 1/3rd of our interval window ago
  // assume it was just missed and report back "the next frame is right now"
